...
[desc]  Returns a uin16 describing which events have occurred since the last time
        fswitchEvents was cleared. The uint16 has a buffering effect, meaining that no
        events will be forgotten until they are cleared by an outside source. This
        function has debouncing built, though it is not strictly necessary in the case
        of float switches.

[ret]   A uint16 representing all the events which have occurred since fswitchEvents was
        last cleared. Use with the FloatSwitchEventFlags bitmasks in this file's header.
*/
uint16 fswitchCheckEvents(void) {
