...
Tank* tank(int volume, int quantity);
Device* device(int enable, int flowRate, int consumption, Tank* from, Tank* to);
void updateMachine(void);
int moveWater(Tank* from, Tank* to, int amount);
void updateDevice(Device* device);
void printTanks(Tank* tank[]);
...
[desc]	Initializes a machine described in two storage arrays. Not yet public
		due to its relative uselessness as a public function due to low-modularity
		of machine implementation.
...
void defaultMachineInit(void) {
	...
	Tank* source = tank(INFINITY, INFINITY);
	Tank* tank2 = tank(100, 0);
	Tank* tank3 = tank(100, 0);
	Tank* tank4 = tank(100, 0);
	Tank* sink = tank(INFINITY, 0);
	...
	/* device(enable, flowRate, powerconsumption, source, sink) */
	//Filter pump
	deviceArr[0] = device(FALSE, 15, 10, source, tank2);
	//RO pump
	deviceArr[1] = device(FALSE, 9, 30, tank2, tank3);
	//UV disinfect
	deviceArr[2] = device(FALSE, 20, 15, tank3, tank4);
	//Slow drain
	deviceArr[3] = device(FALSE, 3, 15, tank4, sink);
...
void updateMachine(void) {
	...
	for(i=0; i < numDevices; i++) {
		if(devices[i]->enable)
			moveWater(devices[i]->from, devices[i]->to, devices[i]->flowRate);
...
[desc]	Moves water from one tank into another. Only moves water if the
		water is availble in the 'from tank'. Will not overfill the 'to tank'.
			Water movement: 'from tank' -> 'to tank'

[from] Tank to source water from.
[to] Tank to sink water into.
[amount] Amount of water to move.

[ret]	0 if 'to tank' was filled, 1 otherwise.
*/
int moveWater(Tank* from, Tank* to, int amount) {
	/* increase sink quantity */
	if (to->volume != INFINITY) { 
		if (from->quantity < amount)
			to->quantity += from->quantity; //source doesn't have enough
		else 
			to->quantity += amount; //source has enough
		if (to->quantity > to->volume)
			to->quantity = to->volume; //Cap volume at full tank
	}

	/* decrease source tank */
	if (from->quantity != INFINITY && from->quantity > 0) {
		from->quantity -= amount;
		if (from->quantity < 0)
			from->quantity = 0; //Cap volume at empty tank
	}
	return isFull(to)? FALSE: TRUE;
}
...
*/
void updateDevice(Device* device) {
	if (device->to->quantity == device->to->volume)
		device->enable = FALSE;
	else if (device->to->quantity <= device->to->volume/2)
		device->enable = TRUE;
}
